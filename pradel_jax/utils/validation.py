...
def validate_positive(
    ...
    if hasattr(value, "shape"):  # Array-like
        if strict:
            condition = (
                jnp.all(value > 0) if hasattr(value, "device") else np.all(value > 0)
            )
            operator = ">"
        else:
            condition = (
...
def validate_probability(
    value: Union[float, np.ndarray, jnp.ndarray], name: str = "probability"
) -> None:
    """
    ...
    if hasattr(value, "shape"):  # Array-like
        condition = (
            jnp.all(value <= 1) if hasattr(value, "device") else np.all(value <= 1)
        )
        if not condition:
...
def validate_capture_matrix(
    ...
    validate_array_dimensions(
        capture_matrix, min_dims=2, max_dims=2, name="capture_matrix"
    )
    ...
    unique_values = (
        jnp.unique(capture_matrix)
        if hasattr(capture_matrix, "device")
        else np.unique(capture_matrix)
    )
    ...
    total_captures = (
        jnp.sum(capture_matrix)
        if hasattr(capture_matrix, "device")
        else np.sum(capture_matrix)
    )
    if total_captures == 0:
        raise ValidationError(
