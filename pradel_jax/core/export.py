...
class ResultsExporter:
    ...
    def export_results(
        ...
        for i, result in enumerate(results):
            model_spec = (
                model_specs[i] if model_specs and i < len(model_specs) else None
            )
            export_record = self._extract_result_data(
                result, model_spec, data_context, i + 1, include_metadata
            )
        ...
        export_df[numeric_columns] = export_df[numeric_columns].round(
    ...
    def _extract_result_data(
        ...
        if not export_record["converged"]:
            export_record["error_message"] = getattr(
        ...
        if (
            self.include_design_info
            and model_spec
            and data_context
            and hasattr(model_spec, "formula_spec")
            and hasattr(result, "parameters")
        ):
            ...
            for param_name, param_value in zip(all_param_names, result.parameters):
                ...
                if hasattr(result, "parameter_se") and result.parameter_se is not None:
                    if len(result.parameter_se) > len(all_param_names):
                        se_value = result.parameter_se[
                            all_param_names.index(param_name.split("_", 1)[1])
                        ]
                        export_record[f"{param_name}_se"] = se_value

                        # MARK-style confidence intervals (estimate ± 1.96 * SE)
                        export_record[f"{param_name}_lcl"] = (
                            param_value - 1.96 * se_value
                        )
                        export_record[f"{param_name}_ucl"] = (
                            param_value + 1.96 * se_value
                        )
    ...
    def create_model_comparison_table(
        ...
        if comparison_col in successful_df.columns:
            successful_df = successful_df.sort_values(
                comparison_col, ascending=ascending
            )
        ...
        if f"{comparison_col}_weight" in successful_df.columns:
            successful_df["cumulative_weight"] = successful_df[
                f"{comparison_col}_weight"
            ].cumsum()

        # Add MARK-style model selection indicators
        successful_df["substantial_support"] = (
            successful_df[delta_col] <= 2.0
        )  # Delta < 2
        successful_df["some_support"] = (successful_df[delta_col] > 2.0) & (
            successful_df[delta_col] <= 7.0
        )  # 2 < Delta <= 7
    ...
    def print_summary(self, export_df: pd.DataFrame) -> None:
        ...
        if len(successful_models) > 0:
            ...
            print(
                comparison_df[available_cols].to_string(
            ...
            # Parameter summary
            param_cols = [
                col for col in export_df.columns if col.startswith(("phi_", "p_", "f_"))
            ]
            if param_cols:
        ...
        if len(failed_models) > 0:
            ...
            available_failed_cols = [
                col for col in failed_cols if col in failed_models.columns
            ]
