    ...
    def get_parameter_summary(self) -> Dict[str, Dict[str, float]]:
        ...
        for i, name in enumerate(self.parameter_names):
            summary[name] = {
                ...
                "z_score": (
                    float(self.estimates[i] / self.standard_errors[i])
                    if self.standard_errors[i] > 0
                    else np.inf
                ),
                "p_value": (
                    2
                    * (
                        1
                        - stats.norm.cdf(
                            abs(self.estimates[i] / self.standard_errors[i])
                        )
                    )
                    if self.standard_errors[i] > 0
                    else 0.0
...
class HessianBasedUncertainty:
    ...
    def compute_uncertainty(
        ...
        if np.any(variances < 0):
            self.logger.warning(
                "Negative variances detected. Setting to small positive value."
            )
    ...
    def _compute_autodiff_hessian(
        self, log_likelihood_fn: Callable[[np.ndarray], float], parameters: np.ndarray
    ) -> np.ndarray:
        ...
        try:
            ...
            self.logger.warning(
                f"Auto-diff Hessian computation failed: {e}. Using finite differences."
            )
            return self._compute_finite_difference_hessian(
                log_likelihood_fn, parameters
            )
...
class BootstrapUncertainty:
    ...
    def compute_bootstrap_uncertainty(
        ...
        self.logger.info(
            f"Computing bootstrap uncertainty with {n_bootstrap_samples} samples"
        )
        ...
        for i in range(n_bootstrap_samples):
            ...
            if (i + 1) % 100 == 0:
                self.logger.info(
                    f"Completed {i+1}/{n_bootstrap_samples} bootstrap samples"
                )
        ...
        self.logger.info(
            f"Successfully completed {successful_samples} bootstrap samples"
        )
...
def bootstrap_confidence_intervals(
    data_context: Any, model_fit_fn: Callable[[Any], Tuple[np.ndarray, float]], **kwargs
) -> ParameterUncertainty:
    """
...
def compute_fisher_information(
    log_likelihood_fn: Callable[[np.ndarray], float], parameters: np.ndarray
) -> np.ndarray:
    """
    ...
    """
    uncertainty_computer = HessianBasedUncertainty()
    hessian_matrix = uncertainty_computer._compute_autodiff_hessian(
        log_likelihood_fn, parameters
    )
    return -hessian_matrix
