...
class RegressionTestDefinition:
    ...
    def generate_test_id(self) -> str:
        """Generate unique test ID based on test configuration using secure SHA-256."""
...
class PerformanceRegressionTester:
    ...
    def __init__(self, baseline_directory: Optional[Path] = None):
        ...
        self.baseline_directory = baseline_directory or Path(
    ...
    def run_regression_test(
        ...
        model_fitter: Callable[
            [Any, Dict[str, str]], Tuple[np.ndarray, float, Dict[str, Any]]
        ],
        ...
        try:
            ...
            if baseline_result is not None:
                parameter_differences = (
                    parameter_estimates - baseline_result.parameter_estimates
                )
                log_likelihood_difference = (
                    log_likelihood - baseline_result.log_likelihood
                )
            ...
            # Check against expected parameters if provided
                ...
                expected_differences = (
                    parameter_estimates - test_definition.expected_parameters
                )
            ...
            self.logger.info(
                f"Test {test_definition.name}: {'PASSED' if passes_tolerance else 'FAILED'}"
            )
            return result
    ...
    def run_test_suite(
        ...
        """
        self.logger.info(
            f"Running regression test suite: {suite_name} ({len(test_definitions)} tests)"
        )
    ...
    def load_baseline_result(
        self, test_def: RegressionTestDefinition
    ) -> Optional[RegressionTestResult]:
    ...
    def save_test_report(
        self, test_suite: RegressionTestSuite, output_path: Path
    ) -> None:
        """Save comprehensive test report."""
        report = {
            "summary": test_suite.get_summary(),
            "detailed_results": [
                result.to_dict() for result in test_suite.test_results
            ],
            "metadata": {
