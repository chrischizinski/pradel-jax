...
class ModelDiagnostics:
    ...
    def compare_with(self, other: "ModelDiagnostics") -> Dict[str, float]:
        ...
        comparison["delta_aic"] = (
            self.selection_criteria.aic - other.selection_criteria.aic
        )
        comparison["delta_aicc"] = (
            self.selection_criteria.aicc - other.selection_criteria.aicc
        )
        comparison["delta_bic"] = (
            self.selection_criteria.bic - other.selection_criteria.bic
        )

        # Log-likelihood difference
        comparison["delta_log_likelihood"] = (
            self.selection_criteria.log_likelihood
            - other.selection_criteria.log_likelihood
        )

        # Akaike weights
        aic_diff = comparison["delta_aic"]
        comparison["akaike_weight_ratio"] = (
            np.exp(-0.5 * aic_diff) if aic_diff != 0 else 1.0
        )
...
class ModelSelectionCriteriaComputer:
    ...
    def compute_criteria(
        ...
        if n_observations > n_parameters + 1:
            correction = (2 * n_parameters * (n_parameters + 1)) / (
...
class GoodnessOfFitTester:
    ...
    def compute_goodness_of_fit(
        ...
        if df <= 0:
            self.logger.warning(
                "Degrees of freedom <= 0. Goodness-of-fit tests may be unreliable."
            )
        ...
        # Residual analysis
        pearson_residuals = self._compute_pearson_residuals(
            observed_data, expected_data
        )
        deviance_residuals = self._compute_deviance_residuals(
            observed_data, expected_data
        )
        ...
        if is_overdispersed:
            self.logger.info(
                f"Model shows evidence of overdispersion (c-hat = {overdispersion_estimate:.3f})"
            )
    ...
    def _compute_pearson_chi_square(
        self, observed: np.ndarray, expected: np.ndarray, df: int
    ) -> Tuple[float, float]:
    ...
    def _compute_deviance_test(
        ...
        mask = observed > 0
        if np.sum(mask) > 0:
            deviance_components = observed[mask] * np.log(
                observed_safe[mask] / expected_safe[mask]
            )
    ...
    def _compute_deviance_residuals(
        ...
        if np.sum(mask) > 0:
            deviance_components[mask] = (
                2 * observed[mask] * np.log(observed_safe[mask] / expected_safe[mask])
            )
    ...
    def _compute_standardized_residuals(
        self, pearson_residuals: np.ndarray, expected: np.ndarray
    ) -> np.ndarray:
...
def compute_complete_model_diagnostics(
    ...
    overdispersion = (
        gof_results.overdispersion_estimate if gof_results.is_overdispersed else None
    )
...
def plot_diagnostic_plots(
    diagnostics: ModelDiagnostics, save_path: Optional[str] = None
):
    """
    ...
    logger.info(
        f"Overdispersion: {diagnostics.goodness_of_fit.overdispersion_estimate:.3f}"
    )
