...
class QualityGateEvaluator:
    ...
    def evaluate_validation_results(
        self, session: Any, validation_results: List[Any]  # ValidationSession
    ) -> QualityGateReport:
        """
        ...
        logger.info(
            f"Quality gate evaluation completed: {overall_decision.value} (score: {overall_score:.3f})"
        )
        return report

    def _assess_parameter_quality(
        ...
        accuracy_status, accuracy_risk = self.thresholds[
            QualityMetric.PARAMETER_ACCURACY
        ].assess_value(accuracy_score)
        ...
        if accuracy_status == "poor":
            ...
            recommendations.append(
                "Review parameter estimation methods and numerical precision"
            )

        # Statistical power assessment
        power_score = parameter_metrics.get("statistical_power", 0.0)
        power_status, power_risk = self.thresholds[
            QualityMetric.STATISTICAL_POWER
        ].assess_value(power_score)
        ...
        weighted_score = (
            metric_scores[QualityMetric.PARAMETER_ACCURACY]
            * self.thresholds[QualityMetric.PARAMETER_ACCURACY].weight
            + metric_scores[QualityMetric.STATISTICAL_POWER]
            * self.thresholds[QualityMetric.STATISTICAL_POWER].weight
        ) / (
            self.thresholds[QualityMetric.PARAMETER_ACCURACY].weight
            + self.thresholds[QualityMetric.STATISTICAL_POWER].weight
        )
        ...
        overall_status, risk_level = self._determine_component_status(
            weighted_score, issues
        )
    ...
    def _assess_model_quality(
        self, session: Any, validation_results: List[Any]
    ) -> ComponentAssessment:
        ...
        concordance_status, concordance_risk = self.thresholds[
            QualityMetric.MODEL_CONCORDANCE
        ].assess_value(concordance_score)
        ...
        if concordance_status == "poor":
            ...
            recommendations.append(
                "Investigate differences in model implementation or optimization"
            )

        # Convergence reliability assessment
        convergence_score = model_metrics.get("convergence_rate", 0.0)
        convergence_status, convergence_risk = self.thresholds[
            QualityMetric.CONVERGENCE_RELIABILITY
        ].assess_value(convergence_score)
        ...
        if convergence_status == "poor":
            ...
            recommendations.append(
                "Review optimization algorithms and initial parameter values"
            )
        ...
        weighted_score = (
            ...
            + convergence_score
            * self.thresholds[QualityMetric.CONVERGENCE_RELIABILITY].weight
        ) / (
            self.thresholds[QualityMetric.MODEL_CONCORDANCE].weight
            + self.thresholds[QualityMetric.CONVERGENCE_RELIABILITY].weight
        )
    ...
    def _assess_system_quality(self, session: Any) -> ComponentAssessment:
        ...
        stability_status, stability_risk = self.thresholds[
            QualityMetric.SYSTEM_STABILITY
        ].assess_value(stability_score)
        ...
        efficiency_status, efficiency_risk = self.thresholds[
            QualityMetric.PERFORMANCE_EFFICIENCY
        ].assess_value(efficiency_score)
        ...
        if efficiency_status == "poor":
            issues.append(
                f"Performance efficiency below threshold: {efficiency_score:.1%}"
            )
        ...
        weighted_score = (
            ...
            + efficiency_score
            * self.thresholds[QualityMetric.PERFORMANCE_EFFICIENCY].weight
        ) / (
            self.thresholds[QualityMetric.SYSTEM_STABILITY].weight
            + self.thresholds[QualityMetric.PERFORMANCE_EFFICIENCY].weight
        )
    ...
    def _extract_parameter_metrics(
        self, validation_results: List[Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_relative_difference": (
                np.mean(relative_differences) if relative_differences else 0.0
            ),
            "max_relative_difference": (
                np.max(relative_differences) if relative_differences else 0.0
            ),
        }
    ...
    def _extract_system_metrics(self, session: Any) -> Dict[str, Any]:
        ...
        stability_rate = max(
            0.0, 1.0 - (error_count + warnings_count * 0.5) / total_operations
        )
    ...
    def _check_critical_parameters(self, validation_results: List[Any]) -> List[str]:
        ...
        for param_name in self.criteria.critical_parameters:
            ...
            if not param_results:
                issues.append(
                    f"Critical parameter '{param_name}' not found in validation results"
                )
                continue
    ...
    def _check_criteria_compliance(
        ...
        param_accuracy = parameter_assessment.metric_scores.get(
            QualityMetric.PARAMETER_ACCURACY, 0.0
        )
    ...
    def _determine_overall_risk(
        self, assessments: List[ComponentAssessment]
    ) -> RiskLevel:
    ...
    def _generate_recommendations(
        ...
        if not all_recommendations:
            all_recommendations.append(
                "Validation completed successfully - system ready for production use"
            )
    ...
    def _generate_approval_conditions(
        self, decision: QualityGateDecision, warnings: List[str], risk_level: RiskLevel
    ) -> List[str]:
