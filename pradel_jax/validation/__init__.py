...
try:
    from .rmark_interface import (
        RMarkExecutor,
        RMarkResult,
        execute_rmark_analysis
    )
...
def get_validation_info() -> Dict[str, Any]:
    ...
    return {
        ...
        ] if _QUALITY_GATES_AVAILABLE else []
    }

# Initialize validation framework
logger.info(f"Pradel-JAX Validation Framework v{__version__} loaded")
logger.info(f"Phase 1 (Core): available")
logger.info(f"Phase 2 (Advanced Statistics): {'available' if _ADVANCED_STATS_AVAILABLE else 'not available'}")
logger.info(f"Phase 3 (Pipeline & Quality Gates): {'complete' if is_phase3_complete() else 'partial'}")
logger.info(f"  - Configuration system: {'available' if _CONFIG_AVAILABLE else 'not available'}")
logger.info(f"  - Pipeline orchestration: {'available' if _PIPELINE_AVAILABLE else 'not available'}")
logger.info(f"  - Quality gates: {'available' if _QUALITY_GATES_AVAILABLE else 'not available'}")
logger.info(f"  - Parallel execution: {'available' if _PARALLEL_EXECUTION_AVAILABLE else 'not available'}")
logger.info(f"  - Error handling: {'available' if _ERROR_HANDLING_AVAILABLE else 'not available'}")
