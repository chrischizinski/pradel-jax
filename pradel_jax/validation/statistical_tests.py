...
class EquivalenceTestResult(StatisticalTestResult):
    ...
    def __post_init__(self):
        ...
        self.test_statistic = max(abs(self.lower_test_statistic), abs(self.upper_test_statistic))
...
def test_parameter_equivalence(
    ...
    """
    logger.debug(f"Testing parameter equivalence: {estimate_1:.4f} ± {std_error_1:.4f} vs {estimate_2:.4f} ± {std_error_2:.4f}")
    ...
    reference_value = max(abs(estimate_1), abs(estimate_2), 1.0)  # Avoid division by very small numbers
    ...
    if is_equivalent:
        result.notes.append(f"Parameters are equivalent within ±{equivalence_margin*100:.1f}% margin")
    else:
        result.notes.append(f"Cannot conclude equivalence within ±{equivalence_margin*100:.1f}% margin")
    ...
    logger.debug(f"TOST result: p={overall_p:.4f}, equivalent={is_equivalent}, effect_size={effect_size:.3f}")
...
def _simple_equivalence_test(
    ...
    return EquivalenceTestResult(
        ...
        notes=[f"Simplified test: relative difference {relative_diff:.4f} vs margin {equivalence_margin:.4f}"]
    )
...
def test_confidence_interval_overlap(
    ...
    """
    logger.debug(f"Testing CI overlap: {estimate_1:.4f}±{std_error_1:.4f} vs {estimate_2:.4f}±{std_error_2:.4f}")
    ...
    if has_overlap:
        if overlap_prop > 0.5:
            ...
            notes = ["Substantial confidence interval overlap suggests compatible estimates"]
        else:
            result = TestResult.INCONCLUSIVE  
            notes = ["Minimal confidence interval overlap - estimates may differ"]
    else:
        result = TestResult.NOT_EQUIVALENT
        notes = ["Non-overlapping confidence intervals suggest statistically significant difference"]
...
def calculate_model_ranking_concordance(
    ...
    logger.debug(f"Ranking concordance: tau={tau:.3f}, p={p_value:.4f}, perfect_rate={perfect_agreement_rate:.3f}")
