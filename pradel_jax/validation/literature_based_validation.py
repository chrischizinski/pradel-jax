...
class LiteratureBasedValidator:
    ...
    def detect_sparse_data_issues(self, 
                                data_context: DataContext,
                                design_matrices: Dict) -> SparseDataDiagnostics:
        """
        ...
        sample_coverage = multiple_captures / total_observed if total_observed > 0 else 0.0
        ...
        if sample_coverage < 0.55:
            recommendations.append("Sample coverage insufficient - use bounds estimation")
    ...
    def triangulate_models(self,
        ...
        design_matrices_sample = model.build_design_matrices(model_specs[0], data_context)
        sparse_diagnostics = self.detect_sparse_data_issues(data_context, design_matrices_sample)
        ...
        if sparse_diagnostics.sparsity_risk == "HIGH":
            assessment = "HIGH RISK: Sparse data detected. Model selection may be unreliable."
        ...
        uncertainty_metrics = {
            ...
            'aic_cv': aic_cv,
            'min_delta_aic': np.min(delta_aics[delta_aics > 0]) if np.sum(delta_aics > 0) > 0 else 0,
        ...
        return ModelTriangulationResult(
            ...
            sparse_cell_warning=(sparse_diagnostics.sparsity_risk in ["HIGH", "MODERATE"]),
    ...
    def cross_validate_with_ecological_splits(self,
                                            model_specs: List[FormulaSpec],
                                            data_context: DataContext,
    ...
    def _generate_random_splits(self, data_context: DataContext, 
                               cv_config: CrossValidationConfig) -> List[Tuple[np.ndarray, np.ndarray]]:
        ...
        for i in range(cv_config.n_folds):
            ...
            end_idx = start_idx + fold_size if i < cv_config.n_folds - 1 else n_individuals
    ...
    def _generate_temporal_splits(self, data_context: DataContext,
                                cv_config: CrossValidationConfig) -> List[Tuple[np.ndarray, np.ndarray]]:
    ...
    def _generate_spatial_splits(self, data_context: DataContext,
                               cv_config: CrossValidationConfig) -> List[Tuple[np.ndarray, np.ndarray]]:
    ...
    def _subset_data_context(self, data_context: DataContext, 
                           indices: np.ndarray) -> DataContext:
        ...
        if hasattr(data_context, 'covariates') and data_context.covariates:
            for covariate_name, covariate_values in data_context.covariates.items():
                if isinstance(covariate_values, (list, np.ndarray)):
                    subset_covariates[covariate_name] = np.array(covariate_values)[indices]
                else:
    ...
    def _calculate_validation_metrics(self,
                                    ...
                                    val_context: DataContext) -> Dict[str, float]:
        """Calculate validation metrics on held-out data."""
        
        # Simplified validation - would need actual prediction implementation
        train_likelihood = triangulation_result.primary_estimates.get('log_likelihood', 0)
    ...
    def _aggregate_cv_results(self, cv_results: List[Dict]) -> Dict[str, Any]:
        ...
        all_train_ll = [r['validation_metrics']['train_log_likelihood_per_obs'] for r in cv_results]
...
def comprehensive_literature_based_validation(
    ...
    sparse_diagnostics = validator.detect_sparse_data_issues(data_context, sample_design_matrices)
...
def _generate_final_recommendations(
    ...
    uncertainty = triangulation_result.uncertainty_metrics['model_selection_uncertainty']
    if uncertainty > 0.4:
