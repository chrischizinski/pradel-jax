...
class EnvironmentDetector:
    ...
    def get_recommended_method(self) -> ExecutionMethod:
        ...
        if (
            self.config.preferred_environment == "ssh"
            and ExecutionMethod.SSH in available
        ):
            return ExecutionMethod.SSH
        elif (
            self.config.preferred_environment == "local_r"
            and ExecutionMethod.LOCAL_R in available
        ):
...
class SSHRMarkExecutor:
    ...
    def __init__(self, config: SecureValidationConfig):
        ...
        if not config.has_ssh_config():
            raise SecurityError(
                "SSH configuration incomplete - check environment variables"
            )
    ...
    def _generate_rmark_script(
        self, data: DataContext, formula_spec: FormulaSpec
    ) -> str:
        ...
        factor_code = (
            "\n    ".join(factor_conversions)
            if factor_conversions
            else "# No factor conversions needed"
        )
...
"""
    ...
    def _transfer_data_and_script(self, ssh, data: DataContext, r_script: str):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
            ...
            for i in range(data.n_individuals):
                ch = "".join(
                    str(int(data.capture_matrix[i, j])) for j in range(data.n_occasions)
                )
        ...
        try:
            ...
            logger.debug(
                f"Transferred files to remote machine: {remote_data_path}, {remote_script_path}"
            )
    ...
    def _execute_remote_script(self, ssh) -> str:
        ...
        if exit_status != 0:
            raise RuntimeError(
                f"R script execution failed (exit code {exit_status}): {stderr_text}"
            )

        logger.debug(f"Remote execution completed with exit status {exit_status}")
        return stdout_text

    def _parse_rmark_output(
        self, output: str, formula_spec: FormulaSpec
    ) -> RMarkResult:
...
class LocalRMarkExecutor:
    ...
    def execute(self, data: DataContext, formula_spec: FormulaSpec) -> RMarkResult:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(
                prefix=f"rmark_{self.session_id}_"
            ) as temp_dir:
                ...
                result = subprocess.run(
                    ...
                    capture_output=True,
                    text=True,
                    timeout=self.config.local_r_timeout,
                )
    ...
    def _generate_local_rmark_script(
        self, data_file: Path, formula_spec: FormulaSpec
    ) -> str:
...
"""
    ...
    def _parse_local_output(
        self, result: subprocess.CompletedProcess, formula_spec: FormulaSpec
    ) -> RMarkResult:
...
class MockRMarkExecutor:
    ...
    def execute(self, data: DataContext, formula_spec: FormulaSpec) -> RMarkResult:
        ...
        execution_time = (
            time.time() - start_time + np.random.uniform(0.1, 0.5)
        )  # Mock execution time
...
class RMarkExecutor:
    ...
    def __init__(self, config: Optional[SecureValidationConfig] = None):
        ...
        logger.info(
            f"RMark executor initialized with methods: {[m.value for m in self.available_methods]}"
        )
    ...
    def execute_rmark_analysis(
        ...
        try:
            ...
            if method != ExecutionMethod.MOCK:
                ...
                result.notes.append(
                    f"Fallback to mock after {method.value} failure: {str(e)}"
                )
                return result
            else:
                raise
