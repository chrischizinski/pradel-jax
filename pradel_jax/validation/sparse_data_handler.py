...
class SparseDataHandler:
    ...
    def assess_sparsity_severity(self, data_context: DataContext) -> Dict[str, Any]:
        ...
        sample_coverage = (
            multiple_detections / (n_individuals - never_detected)
            if (n_individuals - never_detected) > 0
            else 0.0
        )
    ...
    def sample_coverage_estimator(
        ...
        if sample_coverage >= 0.55:  # Sufficient coverage threshold from Wesson paper
            ...
            if n1 > 0:
                upper_bound = n_plus + n1**2 / (
                    2 * max(1, n2)
                )  # Conservative when n2 is small
            else:
    ...
    def conservative_bounds_estimation(
        self, data_context: DataContext
    ) -> SparseDataSolution:
        """
    ...
    def regularized_likelihood_estimation(
        self,
        ...
        def regularized_objective(params):
            ...
            try:
                ...
                log_likelihood = model.log_likelihood(
                    params, data_context, design_matrices
                )
        ...
        if result.success:
            ...
            final_likelihood = model.log_likelihood(
                final_params, data_context, design_matrices
            )
            ...
            detection_prob_est = np.mean(
                np.exp(final_params[: data_context.capture_matrix.shape[1]])
            )  # Simplified
            ...
            population_estimate = (
                np.sum(data_context.capture_matrix.sum(axis=1) > 0) * 1.5
            )
        ...
        if not result.success:
            methodological_notes.append(
                "WARNING: Optimization failed - using fallback estimate"
            )
    ...
    def handle_sparse_scenario(
        self,
        data_context: DataContext,
        formula_specs: Optional[List[FormulaSpec]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        if severity in ["EXTREME"]:
            try:
                solutions["conservative_bounds"] = self.conservative_bounds_estimation(
                    data_context
                )
            except Exception as e:
