...
class ValidationCriteria:
    ...
    critical_parameters: List[str] = field(
        default_factory=lambda: ["phi_intercept", "p_intercept", "f_intercept"]
    )
...
class SecureValidationConfig:
    ...
    def _load_environment_config(self):
        ...
        if port_str := os.getenv("RMARK_SSH_PORT"):
            try:
                ...
                logger.warning(
                    f"Invalid SSH port '{port_str}', using default {self.ssh_port}"
                )
        ...
        if timeout_str := os.getenv("RMARK_SSH_TIMEOUT"):
            try:
                ...
                logger.warning(
                    f"Invalid SSH timeout '{timeout_str}', using default {self.ssh_timeout}"
                )
        ...
        if local_timeout_str := os.getenv("RMARK_LOCAL_R_TIMEOUT"):
            try:
                ...
                logger.warning(
                    f"Invalid local R timeout '{local_timeout_str}', using default {self.local_r_timeout}"
                )

        # Environment preference
        self.preferred_environment = os.getenv(
            "RMARK_PREFERRED_ENVIRONMENT", self.preferred_environment
        )
        ...
        if max_time_str := os.getenv("RMARK_MAX_EXECUTION_TIME"):
            try:
                ...
                logger.warning(
                    f"Invalid max execution time '{max_time_str}', using default"
                )
    ...
    def _validate_security_configuration(self):
        ...
        if not self.security.use_process_specific_temp_dirs:
            security_issues.append(
                "Not using process-specific temp directories (potential security risk)"
            )

        # Check execution time limits
        if self.security.max_execution_time_seconds > 3600:  # 1 hour
            security_issues.append(
                f"Very long max execution time ({self.security.max_execution_time_seconds}s) may indicate security risk"
            )
        ...
        if security_issues:
            logger.warning(
                "Security validation completed with warnings - review configuration"
            )
    ...
    def load_config(self) -> SecureValidationConfig:
        ...
        if "criteria" in config_data:
            ...
            secure_config.criteria = ValidationCriteria(
                parameter_absolute_tolerance=criteria_config.get(
                    "parameter_absolute_tolerance", 1e-3
                ),
                parameter_relative_tolerance_pct=criteria_config.get(
                    "parameter_relative_tolerance_pct", 5.0
                ),
                max_aic_difference=criteria_config.get("max_aic_difference", 2.0),
                max_likelihood_relative_diff_pct=criteria_config.get(
                    "max_likelihood_relative_diff_pct", 1.0
                ),
                min_ranking_concordance=criteria_config.get(
                    "min_ranking_concordance", 0.8
                ),
                min_convergence_rate=criteria_config.get("min_convergence_rate", 0.95),
                min_pass_rate_for_approval=criteria_config.get(
        ...
        if "security" in config_data:
            ...
            secure_config.security = SecuritySettings(
                max_execution_time_seconds=security_config.get(
                    "max_execution_time_minutes", 30
                )
                * 60,
                cleanup_temp_files_on_success=security_config.get(
                    "cleanup_temp_files_on_success", True
                ),
                cleanup_temp_files_on_error=security_config.get(
                    "cleanup_temp_files_on_error", True
                ),
        ...
        if "output" in config_data:
            ...
            secure_config.output_base_dir = output_config.get(
                "base_output_dir", "./validation_results"
            )
            secure_config.generate_html_report = output_config.get(
                "generate_html_report", True
            )
            secure_config.generate_pdf_report = output_config.get(
                "generate_pdf_report", False
            )
            secure_config.save_detailed_results = output_config.get(
        ...
        if "local_r" in config_data:
            ...
            secure_config.auto_install_rmark = local_r_config.get(
