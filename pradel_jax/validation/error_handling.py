...
class CriticalValidationError(ValidationError):
    ...
    def __init__(self, message: str, category: ErrorCategory = ErrorCategory.UNKNOWN, **kwargs):
...
class RetryConfiguration:
    ...
    retry_on_exceptions: List[Type[Exception]] = field(default_factory=lambda: [
        RecoverableValidationError, ConnectionError, TimeoutError
    ])
    
    no_retry_exceptions: List[Type[Exception]] = field(default_factory=lambda: [
        CriticalValidationError, ConfigurationError
    ])
...
class MemoryRecoveryStrategy(ErrorRecoveryStrategy):
    ...
    def can_recover(self, error_context: ErrorContext) -> bool:
        return (
            error_context.category == ErrorCategory.SYSTEM_RESOURCE and
            "memory" in error_context.error_message.lower()
        )
...
class FilePermissionRecoveryStrategy(ErrorRecoveryStrategy):
    ...
    def can_recover(self, error_context: ErrorContext) -> bool:
        return (
            error_context.category == ErrorCategory.PERMISSION and
    ...
    def attempt_recovery(self, error_context: ErrorContext) -> bool:
        ...
        try:
            ...
            logger.info(f"Permission recovery attempted for error {error_context.error_id}")
...
class ValidationErrorHandler:
    ...
    def classify_error(self, exception: Exception, context: Dict[str, Any] = None) -> ErrorContext:
        ...
        logger.error(f"Error classified: {error_context.error_id} - {severity.value} {category.value}")
    ...
    def _determine_error_category(self, exception: Exception) -> ErrorCategory:
        ...
        if any(keyword in error_message for keyword in [
            "config", "configuration", "setting", "parameter not found"
        ]):
            return ErrorCategory.CONFIGURATION
        
        # Data processing errors
        if any(keyword in error_message for keyword in [
            "data", "file not found", "parse", "csv", "json", "invalid format"
        ]):
            return ErrorCategory.DATA_PROCESSING
        
        # Model execution errors
        if any(keyword in error_message for keyword in [
            "convergence", "optimization", "likelihood", "parameter", "model"
        ]):
            return ErrorCategory.MODEL_EXECUTION
        
        # RMark interface errors
        if any(keyword in error_message for keyword in [
            "rmark", "ssh", "r script", "mark"
        ]):
            return ErrorCategory.RMARK_INTERFACE
        
        # System resource errors
        if any(keyword in error_message for keyword in [
            "memory", "disk", "cpu", "resource", "timeout"
        ]):
            return ErrorCategory.SYSTEM_RESOURCE
        
        # Network errors
        if any(keyword in error_message for keyword in [
            "network", "connection", "socket", "timeout", "unreachable"
        ]):
    ...
    def _determine_error_severity(self, exception: Exception, category: ErrorCategory) -> ErrorSeverity:
        ...
        if category in [ErrorCategory.MODEL_EXECUTION, ErrorCategory.RMARK_INTERFACE, 
        ...
        **kwargs
        ...
        for attempt in range(retry_config.max_retries + 1):
            try:
                ...
                if any(isinstance(e, exc_type) for exc_type in retry_config.no_retry_exceptions):
                    logger.error(f"Non-retryable error encountered: {error_context.error_id}")
                    raise e
                
                # Check if we've exhausted retries
                if attempt >= retry_config.max_retries:
                    logger.error(f"Max retries exceeded for error: {error_context.error_id}")
                    break
                ...
                if not any(isinstance(e, exc_type) for exc_type in retry_config.retry_on_exceptions):
                    logger.error(f"Error not configured for retry: {error_context.error_id}")
                    raise e
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recovery_attempted = sum(1 for error in self.error_log if error.recovery_attempted)
            recovery_successful = sum(1 for error in self.error_log if error.recovery_successful)
            ...
            return {
                ...
                "recent_errors": [error.to_dict() for error in self.error_log[-10:]]  # Last 10 errors
            }
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            return error_handler.execute_with_error_handling(
                func, *args, retry_policy=retry_policy, retry_config=retry_config,
