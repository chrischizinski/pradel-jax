    ...
    def update_with_result(self, result: TaskResult) -> None:
        ...
        if result.success:
            ...
            if result.execution_time > 0:
                ...
                self.fastest_task_time = min(self.fastest_task_time, result.execution_time)
                self.slowest_task_time = max(self.slowest_task_time, result.execution_time)
        else:
            self.failed_tasks += 1
        
        # Update memory statistics
        if result.memory_usage_mb > 0:
            self.peak_memory_usage_mb = max(self.peak_memory_usage_mb, result.memory_usage_mb)
...
class ProgressMonitor:
    ...
    def _print_progress(self) -> None:
        ...
        print(f'\r[{bar}] {progress:.1%} ({total_processed}/{self.total_tasks}) '
              f'✓{self.completed_tasks} ✗{self.failed_tasks} {remaining_str}', end='', flush=True)
...
class ResourceMonitor:
    ...
    def get_resource_summary(self) -> Dict[str, Any]:
        ...
        return {
            "peak_cpu_percent": max(self.cpu_usage_history) if self.cpu_usage_history else 0,
            "average_cpu_percent": np.mean(self.cpu_usage_history) if self.cpu_usage_history else 0,
            "peak_memory_mb": max(self.memory_usage_history) if self.memory_usage_history else 0,
            "average_memory_mb": np.mean(self.memory_usage_history) if self.memory_usage_history else 0,
...
class ParallelValidationExecutor:
    ...
    def __init__(self, config: PerformanceConfig):
        ...
        logger.info(f"Parallel executor initialized: {self.optimal_workers} workers, "
    ...
    def execute_parallel_validation(
        ...
        logger.info(f"Executing {len(tasks)} validation tasks with {self.optimal_workers} workers")
        ...
        try:
            ...
            logger.info(f"Parallel execution completed: {self.stats.success_rate:.1%} success rate, "
    ...
    def _execute_with_multiprocessing(
        ...
        with ProcessPoolExecutor(max_workers=self.optimal_workers) as executor:
            ...
            for future in as_completed(future_to_task, timeout=self.config.total_pipeline_timeout_seconds):
    ...
    def _execute_with_threading(
        ...
        with ThreadPoolExecutor(max_workers=self.optimal_workers) as executor:
            # Submit all tasks
            future_to_task = {
                executor.submit(self._execute_single_task, validation_function, task): task
                for task in tasks
            }
    ...
    @staticmethod
    def _execute_single_task(validation_function: Callable, task: Dict[str, Any]) -> TaskResult:
    ...
    def execute_with_retry(
        ...
        while retry_count < max_retries:
            ...
            logger.info(f"Retrying {len(failed_tasks)} failed tasks (attempt {retry_count + 1}/{max_retries})")
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timing_analysis": {
                ...
                "fastest_task_time": self.stats.fastest_task_time if self.stats.fastest_task_time != float('inf') else 0,
