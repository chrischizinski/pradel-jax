...
class ValidationCriteria:
    ...
    def validate_criteria(self) -> None:
        ...
        if not 0 < self.parameter_relative_tolerance_pct < 100:
            raise ValueError(
                "Parameter relative tolerance must be between 0 and 100 percent"
            )
...
class ValidationPipelineConfig:
    ...
    secure_config: SecureValidationConfig = field(
        default_factory=SecureValidationConfig
    )
    ...
    def from_environment(
        ...
        logger.info(
            f"Validation pipeline configuration loaded for {config.environment.value}"
        )
        return config
    ...
    @classmethod
    def _load_from_file(
        cls, config_path: Path, base_config: "ValidationPipelineConfig"
    ) -> "ValidationPipelineConfig":
    ...
    def _load_from_environment(
        cls, config: "ValidationPipelineConfig"
    ) -> "ValidationPipelineConfig":
        ...
        env_mappings = {
            ...
            "PRADEL_JAX_ENABLE_QUALITY_GATES": (
                "enable_quality_gates",
                lambda x: x.lower() == "true",
            ),
            "PRADEL_JAX_PARAMETER_TOLERANCE": (
                "validation_criteria.parameter_relative_tolerance_pct",
                float,
            ),
            "PRADEL_JAX_MIN_PASS_RATE": (
                "validation_criteria.min_pass_rate_for_approval",
                float,
            ),
        }
    ...
    def _merge_config_dict(
        config: "ValidationPipelineConfig", config_dict: Dict[str, Any]
    ) -> "ValidationPipelineConfig":
