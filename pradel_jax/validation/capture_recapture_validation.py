...
def calculate_aic_model_weights(
...
def perform_model_averaging(
    ...
    successful_models = [
        (result, spec)
        for result, spec in zip(model_results, model_specs)
    ...
    lambda_estimates = np.array(
        [
            result.lambda_mean
            for result, _ in successful_models
    ...
    if len(lambda_estimates) > 0:
        ...
        lambda_variance = np.sum(
            weights * (lambda_estimates - weighted_lambda_mean) ** 2
        )
    ...
    selection_summary = {
        ...
        "interpretation": _interpret_model_selection_uncertainty(
            model_selection_uncertainty, len(successful_models)
        ),
    }

    logger.info(
        f"Model averaging complete: {len(substantial_support)}/{len(successful_models)} models have substantial support"
    )
    logger.info(
        f"Best model weight: {np.max(weights):.3f}, Selection uncertainty: {model_selection_uncertainty:.3f}"
    )
...
def bootstrap_model_parameters(
    ...
    logger.warning(
        "Bootstrap is computationally intensive - consider smaller n_bootstrap for testing"
    )
    ...
    logger.warning(
        "Simplified bootstrap implementation - not suitable for production use"
    )
    ...
    for i in range(min(n_bootstrap, 100)):  # Limit for demo
        ...
        bootstrap_indices = np.random.choice(
            n_individuals, size=n_individuals, replace=True
        )
    ...
    logger.info(
        f"Bootstrap complete: bias = {bias_estimate:.4f}, 95% CI = [{ci_lower:.3f}, {ci_upper:.3f}]"
    )
...
def goodness_of_fit_test(
    ...
    fitted_ll = model.log_likelihood(
        np.array(fitted_result.parameters), data_context, design_matrices
    )
...
def comprehensive_model_validation(
    ...
    if len(successful_results) < 2:
        logger.warning(
            "Less than 2 models fitted successfully - limited validation possible"
        )
        return results
    ...
    try:
        averaging_result = perform_model_averaging(
            model_results, model_specs, validation_context
        )
    ...
    if len(successful_results) > 0:
        ...
        for i, result in enumerate(model_results):
            if (
                result
                and result.success
                and result.aic == successful_results[best_model_idx].aic
            ):
        ...
        if best_model_spec and n_bootstrap > 0:
            try:
                bootstrap_result = bootstrap_model_parameters(
                    best_model_spec, data_context, n_bootstrap=n_bootstrap
                )
    ...
    if include_gof_tests and len(successful_results) > 0:
        ...
        for i, result in enumerate(successful_results[:3]):  # Test top 3 models
            try:
                ...
                spec_idx = next(
                    j
                    for j, r in enumerate(model_results)
                    if r and r.success and r.aic == result.aic
                )

                gof_result = goodness_of_fit_test(
                    model_specs[spec_idx], data_context, result
                )
...
def _create_validation_summary(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if results.get("model_averaging"):
        ...
        summary["model_selection_uncertainty"] = avg_result.selection_summary[
            "interpretation"
        ]
    ...
    if results.get("goodness_of_fit"):
        ...
        summary["models_with_adequate_fit"] = sum(
            1 for gof in gof_results if gof.model_fits_data
        )
