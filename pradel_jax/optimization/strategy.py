...
class ProblemAnalyzer:
    ...
    def classify_difficulty(self, characteristics: ModelCharacteristics) -> ProblemDifficulty:
    ...
    def predict_performance(
        ...
        if data.get('requires_multiple_devices', False):
            ...
            try:
                ...
                if device_count < 2:
                    success_rate *= 0.2  # Heavy penalty for distributed without multiple devices
            except:
                success_rate *= 0.1
        ...
        # Size scaling
        size_factor = (characteristics.n_individuals * characteristics.n_parameters / 10000) ** data['size_scaling']
        estimated_time = base_time * max(1.0, size_factor)
        
        # Adjust time for large-scale methods
        if n_individuals > 100000:
            if strategy in [OptimizationStrategy.MINI_BATCH_SGD, OptimizationStrategy.STREAMING_ADAM]:
...
class StrategySelector:
    ...
    def select_strategy(
        ...
        logger.debug(f"Problem analysis: {difficulty.value} difficulty, "
        ...
        for strategy in candidates:
            success_rate, time_est = self.predictor.predict_performance(strategy, characteristics)
        ...
        best_strategy, success_rate, time_est, best_score = max(evaluations, key=lambda x: x[3])
        ...
        fallbacks = [s for s, _, _, _ in sorted(evaluations, key=lambda x: x[3], reverse=True)[1:4]]
        preprocessing_recs = self._generate_preprocessing_recommendations(characteristics)
        
        confidence = self._calculate_confidence(characteristics, success_rate, difficulty)
    ...
    def _get_candidate_strategies(
        ...
        # Medium-scale datasets (10k-100k individuals) 
            ...
            if difficulty in [ProblemDifficulty.DIFFICULT, ProblemDifficulty.VERY_DIFFICULT]:
                return [
                    OptimizationStrategy.MINI_BATCH_SGD,
                    OptimizationStrategy.GPU_ACCELERATED,
                    OptimizationStrategy.MULTI_START,
    ...
    def _generate_config(
        ...
        base_configs = {
            OptimizationStrategy.SCIPY_LBFGS: OptimizationConfig(
                max_iter=1000, tolerance=1e-8, init_scale=0.1
            ),
        ...
        if characteristics.condition_estimate and characteristics.condition_estimate > 1e10:
            overrides.update({
    ...
    def _generate_rationale(
        ...
        size_desc = "large" if characteristics.n_individuals > 5000 else "medium" if characteristics.n_individuals > 1000 else "small"
        ...
        if characteristics.condition_estimate and characteristics.condition_estimate > 1e8:
    ...
    def _generate_preprocessing_recommendations(
        ...
        if characteristics.parameter_ratio > 0.1:
            recommendations.append("Consider reducing model complexity or increasing sample size")
        
        if characteristics.data_sparsity > 0.9:
            recommendations.append("Very sparse data - consider data aggregation or model simplification")
        
        if characteristics.condition_estimate and characteristics.condition_estimate > 1e10:
            recommendations.append("Poor numerical conditioning - check for collinear covariates")
        
        if characteristics.covariate_correlation > 0.9:
            recommendations.append("High covariate correlation detected - consider variable selection")
    ...
    def _calculate_confidence(
        ...
        if characteristics.parameter_ratio > 0.15:
            confidence *= 0.8
        if characteristics.condition_estimate and characteristics.condition_estimate > 1e12:
            confidence *= 0.7
...
class OptimizationOrchestrator:
    ...
    def optimize_model(
        ...
        try:
            ...
            for fallback_strategy in recommendation.fallback_strategies:
                ...
                try:
                    fallback_config = self.selector._generate_config(fallback_strategy, 
                    ...
                    logger.warning(f"Fallback strategy {fallback_strategy.value} failed: {fallback_error}")
                    continue
            else:
    ...
    def _run_scipy_lbfgs(self, objective_function, config: OptimizationConfig, context: ModelContext) -> Dict[str, Any]:
    ...
    def _run_jax_adam(self, objective_function, config: OptimizationConfig, context: ModelContext) -> Dict[str, Any]:
    ...
    def _run_multi_start(self, objective_function, config: OptimizationConfig, context: ModelContext) -> Dict[str, Any]:
...
def diagnose_optimization_difficulty(context: ModelContext) -> Dict[str, Any]:
    ...
    return {
        ...
        'recommendations': StrategySelector()._generate_preprocessing_recommendations(characteristics)
    }