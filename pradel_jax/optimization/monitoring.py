...
class PerformanceMonitor:
    ...
    def end_session(self, result: OptimizationResult) -> None:
        ...
        logger.info(f"Ended session {self.active_session}: "
    ...
    def _check_performance_alerts(self, metrics: OptimizationMetrics) -> None:
        ...
        if metrics.stagnation_count > self.thresholds['max_stagnation']:
            alerts.append(f"Optimization stagnating ({metrics.stagnation_count} iterations)")
        
        if metrics.gradient_norm and metrics.gradient_norm > self.thresholds['max_gradient_norm']:
        ...
        if metrics.relative_improvement and metrics.relative_improvement < self.thresholds['min_improvement_rate']:
    ...
    def get_session_summary(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        gradient_norms = [m.gradient_norm for m in metrics if m.gradient_norm is not None]
        ...
        summary.update({
            'convergence_stats': {
                ...
                'total_improvement': objective_values[0] - objective_values[-1] if len(objective_values) > 1 else 0,
            ...
            'performance_stats': {
                'avg_time_per_iteration': session.duration / max(session.total_iterations, 1),
    ...
    def _estimate_convergence_rate(self, metrics: List[OptimizationMetrics]) -> Optional[float]:
...
class ExperimentTracker:
    ...
    def _analyze_strategy_performance(self, comparison_data: List[Dict]) -> Dict[str, Any]:
...
class OptimizationProfiler:
    ...
    def end_profiling(self) -> Dict[str, Any]:
        ...
        for section_data in analysis.values():
            section_data['percentage'] = (section_data['total_time'] / total_time * 100) if total_time > 0 else 0
        ...
        result = {
            ...
            'bottlenecks': [name for name, data in sorted_analysis.items() 
                          if data['percentage'] > 20]  # >20% of total time
        }
