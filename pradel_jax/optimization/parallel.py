        ...
        if result.success:
            ...
            try:
                if hasattr(model, 'calculate_lambda'):
                    lambda_estimates = model.calculate_lambda(opt_result.x, data_context, design_matrices)
                    lambda_summary = model.get_lambda_summary(lambda_estimates)
            except Exception as e:
                logger.warning(f"Failed to calculate lambda estimates: {e}")
            
            # Generate data hash for reproducibility
            import hashlib
...
class ParallelOptimizer:
    ...
    def fit_models_parallel(
        ...
        remaining_specs = [spec for spec in model_specs if spec.index not in completed_indices]
        ...
        for batch_start in range(0, len(remaining_specs), batch_size):
            ...
            logger.info(f"Processing batch: models {batch_specs[0].index+1}-{batch_specs[-1].index+1}")
            ...
            if completed_count > 0:
                ...
                logger.info(f"Batch completed in {batch_time:.1f}s ({models_per_second:.1f} models/sec)")
                logger.info(f"Progress: {completed_count}/{len(model_specs)} ({completed_count/len(model_specs)*100:.1f}%)")
        ...
        for p in p_formulas:
            for f in f_formulas:
                ...
                spec = ParallelModelSpec(
                    ...
                    formula_spec=pj.create_simple_spec(phi=phi, p=p, f=f, name=f"model_{index}"),
                    index=index,
