...
class AdaptiveJAXAdamOptimizer(BaseOptimizer):
    ...
    def minimize(
        ...
        **kwargs,
        ...
        if HAS_OPTAX:
            return self._minimize_with_optax(
                objective, gradient, x0, bounds, start_time
            )
        else:
            return self._minimize_basic_adaptive(
                objective, gradient, x0, bounds, start_time
            )
    ...
    def _create_learning_rate_schedule(self, n_steps: int) -> Callable:
        ...
        if self.config.lr_schedule == "cosine_decay":
            ...
            logger.warning(
                f"Unknown LR schedule: {self.config.lr_schedule}, using constant"
            )
            return optax.constant_schedule(self.config.learning_rate)
    ...
    def _minimize_with_optax(
        ...
        for iteration in range(self.config.max_iter):
            ...
            if grad_norm < self.config.tolerance:
                logger.info(
                    f"Adaptive Adam converged at iteration {iteration} (grad_norm={grad_norm:.2e})"
                )
                break
            ...
            if patience_counter >= self.config.patience:
                logger.info(
                    f"Early stopping at iteration {iteration} (patience exceeded)"
                )
            ...
            # Warm restart check
            if (
                self.config.use_warm_restarts
                and iteration - last_restart >= self.config.restart_period
                and plateau_counter >= self.config.plateau_patience
            ):

                logger.info(f"Warm restart at iteration {iteration}")

                # Reset optimizer state for restart
                optimizer = self._create_gradient_transform(
                    self.config.max_iter - iteration
                )
            ...
            if self.config.verbose and iteration % 100 == 0:
                ...
                logger.info(
                    f"Iteration {iteration}: loss={loss_val:.6f}, "
    ...
    def _project_gradients_to_bounds(
        self, params: jnp.ndarray, grads: jnp.ndarray, bounds: List[Tuple[float, float]]
    ) -> jnp.ndarray:
    ...
    def _project_to_bounds(
        self, params: jnp.ndarray, bounds: List[Tuple[float, float]]
    ) -> jnp.ndarray:
    ...
    def _minimize_basic_adaptive(
        ...
        for t in range(1, self.config.max_iter + 1):
            ...
            if self.config.use_adaptive_lr and t % 100 == 0:
                current_lr *= lr_decay
                current_lr = max(
                    current_lr, self.config.learning_rate * self.config.min_lr_factor
                )
            ...
            if self.config.verbose and t % 100 == 0:
                logger.info(
                    f"Iteration {t}: loss={loss_val:.6f}, "
...
def create_optimized_adam_config(
    problem_size: int, n_parameters: int, difficulty: str = "moderate"
) -> AdaptiveAdamConfig:
    """
