...
from .optimizers import create_optimizer, OptimizationResult, minimize_with_strategy
...
class CircuitBreaker:
    ...
    def record_failure(self, strategy: OptimizationStrategy):
        ...
        if self.failure_counts[strategy] >= self.failure_threshold:
            logger.warning(
                f"Circuit breaker opened: disabling {strategy.value} "
...
class OptimizationOrchestrator:
    ...
    def optimize(
        self, request: OptimizationRequest, context: ModelContext
        ...
        try:
            ...
            if request.enable_monitoring:
                ...
                session_id = self.performance_monitor.start_session(
                    recommendation.strategy, context.n_parameters, difficulty
                )
    ...
    def batch_optimize(
        ...
        """
        with optimization_experiment(
            experiment_name, f"Batch of {len(requests)} optimizations"
        ):
            responses = []
    ...
    def compare_strategies(
        ...
        with optimization_experiment(
            experiment_name, f"Comparing {len(strategies)} strategies"
        ):
            for strategy in strategies:
                ...
                try:
                    ...
                    results[strategy.value] = self._create_error_response(
                        str(e), time.time()
                    )
    ...
    def _execute_with_monitoring(
        ...
        if self.circuit_breaker is None or self.circuit_breaker.can_execute(
            ...
            for fallback_strategy in recommendation.fallback_strategies:
                ...
                try:
                    ...
                    with self.profiler.profile_section(
                        f"fallback_{fallback_strategy.value}"
                    ):
                        fallback_config = self.strategy_selector._generate_config(
                            fallback_strategy,
                            self.strategy_selector.analyzer.analyze_characteristics(
                                context
                            ),
                        )
                    ...
                    logger.warning(
                        f"Fallback strategy {fallback_strategy.value} failed: {e}"
                    )
                    if self.circuit_breaker:
    ...
    def _wrap_objective_with_monitoring(
        self, objective: Callable, session_id: Optional[str], profile_id: Optional[str]
    ) -> Callable:
    ...
    def _generate_response(
        ...
        recommendations = self._generate_recommendations(
            result, recommendation, session_summary
        )
    ...
    def _assess_difficulty(self, context: ModelContext) -> str:
        ...
        characteristics = self.strategy_selector.analyzer.analyze_characteristics(
            context
        )
        difficulty = self.strategy_selector.analyzer.classify_difficulty(
            characteristics
        )
        return difficulty.value

    def _assess_convergence_quality(
        self, result: OptimizationResult, recommendation: StrategyRecommendation
    ) -> str:
    ...
    def _generate_recommendations(
        ...
        if not result.success:
            recommendations.append(
                "Optimization failed - consider preprocessing data or simplifying model"
            )
            recommendations.extend(recommendation.preprocessing_recommendations)

        if result.nit >= recommendation.config.max_iter * 0.9:
            recommendations.append(
                "Reached iteration limit - consider increasing max_iter or improving initialization"
            )
        ...
        if "convergence_stats" in session_summary:
            ...
            if conv_stats.get("total_improvement", 0) < 1e-6:
                recommendations.append(
                    "Very small improvement - check for numerical precision issues"
                )

        return recommendations

    def _create_manual_recommendation(
        self, strategy: OptimizationStrategy, context: ModelContext
    ) -> StrategyRecommendation:
