...
def determine_tier_status(data: pd.DataFrame, tier_columns: Optional[List[str]] = None) -> pd.Series:
    """
    ...
    if tier_columns is None:
        ...
        if not tier_columns:
            logger.warning("No tier columns found - checking for common tier column names")
            # Check for common variations
            common_tier_names = ['tier', 'tier_1', 'tier_2', 'tier_status', 'license_tier']
            tier_columns = [col for col in data.columns if any(name in col.lower() for name in common_tier_names)]
...
def stratified_sample(
    ...
    if n_samples >= len(data):
        logger.info(f"Requested {n_samples} samples from {len(data)} rows - returning all data")
        return data.copy()
    ...
    logger.info(f"Population composition: {len(tier_2_data)} ever tier 2, {len(tier_1_only_data)} tier 1 only")
    ...
    if n_tier_2_target > n_tier_2_available:
        logger.warning(f"Not enough tier 2 individuals ({n_tier_2_available} available, {n_tier_2_target} requested)")
        ...
        logger.warning(f"Not enough tier 1 individuals ({n_tier_1_available} available, {n_tier_1_target} requested)")
    ...
    logger.info(f"Sampling {n_tier_2_actual} tier 2 and {n_tier_1_actual} tier 1 individuals")
    ...
    if n_tier_1_actual > 0:
        tier_1_sample = tier_1_only_data.sample(n=n_tier_1_actual, random_state=random_state)
    ...
    logger.info(f"Final sample: {len(result)} individuals ({final_tier_2_count} tier 2, {final_tier_1_count} tier 1, {final_tier_2_prop:.1%} tier 2)")
...
def train_validation_split(
    ...
    if random_state is not None:
        train_data = train_data.sample(frac=1.0, random_state=random_state).reset_index(drop=True)
        val_data = val_data.sample(frac=1.0, random_state=random_state+1).reset_index(drop=True)
...
def get_sampling_summary(data_context: DataContext) -> Dict[str, any]:
    ...
    """
    if data_context.metadata is None or not data_context.metadata.get('sampling_applied', False):
    ...
    return {
        ...
        'sampling_ratio': (data_context.metadata.get('sampled_n_individuals', 0) / 
                          data_context.metadata.get('original_n_individuals', 1)),
        'tier_2_proportion_target': data_context.metadata.get('tier_2_proportion_target'),
