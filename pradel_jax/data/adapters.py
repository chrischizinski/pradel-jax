    ...
    def process(self, data: pd.DataFrame) -> DataContext:
        ...
        for name, array in covariates.items():
            ...
            if name.endswith("_categories"):
                # Store categorical metadata in the separate metadata dict
                metadata[name] = array
            elif name.endswith("_is_categorical"):
                # Store categorical flags in metadata
                metadata[name] = array
            else:
                # Only convert numeric covariate data to JAX arrays
                try:
        ...
        logger.info(
            ...
            f"{len(covariates_jax)} numeric covariates, {len(metadata)-1} metadata items"
        )
        ...
        for col in covariate_cols:
            if data[col].dtype == "object" or str(data[col].dtype) == "category":
                ...
                if np.any(np.isnan(values)):
                    logger.warning(
                        f"Column '{col}' contains NaN values - filling with mean"
                    )
        ...
        for col in covariate_cols:
            ...
            if is_categorical:
                ...
                for dummy_col in dummies.columns:
                    ...
                    level_name = (
                        dummy_col.split("_", 1)[1] if "_" in dummy_col else dummy_col
                    )
...
class GenericFormatAdapter(DataFormatAdapter):
    ...
    def detect_format(self, data: pd.DataFrame) -> bool:
        ...
        y_cols = [
            col for col in data.columns if col.startswith("Y") and col[1:].isdigit()
        ]
    ...
    def extract_covariates(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        if self.covariate_columns:
            ...
            covariate_cols = [
                col for col in data_processed.columns if col not in exclude_cols
            ]
    ...
    def _preprocess_categorical_variables(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "gender" in data_fixed.columns:
            if data_fixed["gender"].dtype in ["float64", "int64", "float32", "int32"]:
                logger.info(
                    "Converting gender from numeric codes to categorical labels"
                )
                gender_mapping = {1.0: "Male", 2.0: "Female"}
                data_fixed["gender"] = (
                    data_fixed["gender"].fillna(1.0).map(gender_mapping)
                )
        ...
        if "tier_history" in data_fixed.columns:
            if data_fixed["tier_history"].dtype in [
                "float64",
                "int64",
                "float32",
                "int32",
                ...
                logger.info(
                    "Converting tier_history from numeric codes to simplified categories"
                )
                ...
                data_fixed["tier_history"] = data_fixed["tier_history"].apply(
                    simplify_tier_history
                )
            ...
            if data_fixed["tier"].dtype in ["float64", "int64", "float32", "int32"]:
                ...
                def map_tier(value):
                    ...
                    if value == 0:
                        ...
                        logger.warning(
                            f"Unexpected tier value: {value}, mapping to No_Permit"
                        )
                        return "No_Permit"
                ...
                data_fixed["permit_tier_choice"] = data_fixed["tier"].apply(
                    lambda x: (
                        "Tier_1"
                        if x == "Tier_1_Permit"
                        else "Tier_2" if x == "Tier_2_Permit" else "No_Permit"
                    )
                )
            ...
            if var in data_fixed.columns:
                if data_fixed[var].dtype in ["float64", "int64", "float32", "int32"]:
                    ...
                    if std_val > 0:
                        ...
                        logger.info(
                            f"Standardized {var}: mean={mean_val:.2f}, std={std_val:.2f}"
                        )
...
def load_data(
    file_path: Union[str, Path], adapter: Optional[DataFormatAdapter] = None, **kwargs
) -> DataContext:
    """
