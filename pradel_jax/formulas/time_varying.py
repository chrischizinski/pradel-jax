...
class TimeVaryingCovariateDetector:
    ...
    def detect_time_varying_patterns(
        self, data: pd.DataFrame
    ) -> Dict[str, TimeVaryingCovariateInfo]:
        """
        ...
        self.logger.info(
            f"Detected {len(time_varying_covariates)} time-varying covariate groups"
        )

        return time_varying_covariates

    def _detect_underscore_pattern(
        self, data: pd.DataFrame
    ) -> Dict[str, TimeVaryingCovariateInfo]:
        ...
        for base_name, year_cols in base_names.items():
            if len(year_cols) >= 2:  # Need at least 2 time points
                ...
                if is_categorical:
                    ...
                    all_values = (
                        pd.concat([data[col] for col in cols]).dropna().unique()
                    )
                ...
                self.logger.debug(
                    f"Detected time-varying: {base_name} with {len(years)} occasions"
                )
...
class TimeVaryingDesignMatrixBuilder:
    ...
    def _build_numeric_time_varying_matrix(
        ...
        for occasion in parameter_occasions:
            ...
            if np.any(np.isnan(column_data)):
                # Fill with individual mean or overall mean (JAX-compatible)
    ...
    def expand_data_context_with_time_varying(
        self, data_context: Any, data: pd.DataFrame  # DataContext
        ...
        for base_name, tv_info in time_varying_covariates.items():
            ...
            updated_covariates[f"{base_name}_time_varying"] = jnp.array(
                tv_info.data_matrix
            )
        ...
        self.logger.info(
            f"Added {len(time_varying_covariates)} time-varying covariates to context"
        )
        ...
        return type(data_context)(
            ...
            metadata={
                **(data_context.metadata or {}),
                "has_time_varying_covariates": True,
            },
        )


def detect_and_process_time_varying_covariates(
    data: pd.DataFrame, data_context: Any
