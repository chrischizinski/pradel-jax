...
class FormulaParser:
    ...
    def parse(self, formula_string: str) -> ParsedFormula:
        ...
        has_intercept = any(term.__class__.__name__ == 'InterceptTerm' for term in terms)
    ...
    def parse_model_spec(
        ...
        try:
            phi = self.create_parameter_formula(ParameterType.PHI, spec_dict.get("phi", "~1"))
...
def create_simple_spec(
    ...
    """
    parser = FormulaParser()
    return parser.parse_model_spec(
        {"phi": phi, "p": p, "f": f},
        name=name
    )
